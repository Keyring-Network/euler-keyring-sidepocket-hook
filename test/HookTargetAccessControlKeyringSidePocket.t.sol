...
/// @title Mock EVC contract for testing
contract MockEVC {
...
/// @title Mock EVault Factory for testing
...
/// @title Mock Keyring contract for testing
...
/// @title Mock EVault contract for testing
contract MockEVault {
...
/// @title Test suite for HookTargetAccessControlKeyringSidePocket
contract HookTargetAccessControlKeyringSidePocketTest is Test {
    ...
    event CumulativeWithdrawalLiquidityIndexSet(
        uint256 indexed assetsAvailableForWithdrawal,
        uint256 indexed totalSuppliedAssets
    );
    ...
    /// @notice Test setting cumulative withdrawal liquidity index
    ...
    /// @notice Test setting index with zero assets available reverts
    ...
    /// @notice Test setting index with zero total supplied reverts
    ...
    /// @notice Test setting index with both zeros reverts
    ...
    /// @notice Test only admin can set cumulative withdrawal liquidity index
    ...
    /// @notice Test withdrawal within allowed limit
    ...
    /// @notice Test withdrawal exceeding limit reverts
    function test_Withdraw_ExceedsLimit_Reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                HookTargetAccessControlKeyringSidePocket.WithdrawalAmountExceedsLimit.selector,
                600_000e18,
                500_000e18
            )
        );
    ...
    /// @notice Test cumulative withdrawals tracking
    ...
    /// @notice Test redeem within allowed limit
    ...
    /// @notice Test redeem exceeding limit reverts
    ...
    /// @notice Test multiple users with different balances
    ...
    /// @notice Test scenario from requirements doc - liquidity capture prevention
    ...
    /// @notice Test transfer functions are disabled
    ...
    /// @notice Test transferFrom is disabled
    function test_TransferFrom_Disallowed() public {
        vm.expectRevert(HookTargetAccessControlKeyringSidePocket.Disallowed.selector);
        hookTarget.transferFrom(user1, user2, 100e18);
    }

    /// @notice Test transferFromMax is disabled
    function test_TransferFromMax_Disallowed() public {
        vm.expectRevert(HookTargetAccessControlKeyringSidePocket.Disallowed.selector);
        hookTarget.transferFromMax(user1, user2);
    }

    /// @notice Test withdrawal with shares-to-assets conversion (non 1:1 ratio)
    function test_Withdraw_NonOneToOneRatio() public {
    ...
    /// @notice Test redeem with shares-to-assets conversion
    function test_Redeem_NonOneToOneRatio() public {
    ...
    /// @notice Test updating withdrawal index affects limits
    ...
    /// @notice Test edge case: 100% available for withdrawal
    ...
    /// @notice Test edge case: minimal available for withdrawal
    ...
    /// @notice Test authentication still works (inherited from parent)
    ...
    /// @notice Test fuzz: various withdrawal amounts and ratios
    function testFuzz_Withdraw_VariousAmounts(
        uint256 userBalance,
        ...
        userBalance = bound(userBalance, 1e18, 1_000_000e18);
        totalAssets = bound(totalAssets, userBalance, 10_000_000e18);
        availableAssets = bound(availableAssets, 1e18, totalAssets);
        withdrawAmount = bound(withdrawAmount, 1, userBalance);
        
        // Setup
        targetDebtVault.setBalance(user1, userBalance);
        targetDebtVault.setTotalAssets(totalAssets);
        targetDebtVault.setTotalSupply(totalAssets); // 1:1 for simplicity
        
        vm.prank(admin);
        hookTarget.setCumulativeWithdrawalLiquidityIndex(availableAssets, totalAssets);
        
        keyring.setCredential(user1, POLICY_ID, true);
        
        // Calculate expected max withdrawal
        uint256 maxWithdrawable = (availableAssets * userBalance) / totalAssets;
        
        // Try withdrawal
        vm.prank(vault);
        if (withdrawAmount <= maxWithdrawable) {
            hookTarget.withdraw(withdrawAmount, user1, user1);
            assertEq(hookTarget.userWithdrawnAmounts(user1), withdrawAmount);
        } else {
            vm.expectRevert();
            hookTarget.withdraw(withdrawAmount, user1, user1);
        }
